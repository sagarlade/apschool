...
const combinedSubjects = [
  { id: 'combined-marathi-english', name: 'Marathi + Maths' },
    { id: 'combined-maths-itsc', name: 'बुद्धिमत्ता चाचणी + English' }
];
...
export default function MarksEntryForm() {
  ...
  const filteredSubjects = useMemo(() => {
    ...
    if (isScholarshipExam) {
        if (selectedIds.marksType === '150') {
            return combinedSubjects;
        } else if (selectedIds.marksType === '20') {
    ...
    return subjects;
  }, [selectedClass, selectedExam, allSubjects, selectedIds.marksType]);
  ...
  useEffect(() => {
    const fetchStudentsAndMarks = async () => {
      ...
      try {
        ...
        const finalStudents: StudentWithMarks[] = studentData.map((s) => ({ ...s, marks: null, status: 'Pending' as const, isDirty: false }));
  ...
  const handleExport = () => {
    ...
    const csvContent = "data:text/csv;charset=utf-8,"
      ...
      + studentsWithMarks.map(s => `"${s.name}",${s.marks || ''},"${s.status}"`).join("\n");
